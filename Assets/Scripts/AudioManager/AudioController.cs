...
public class AudioController : SingletonMonoBehaviour<AudioController>
{
    ...
    public AudioObject PlayRecentlyPlayedPlaylistTrack( string trackID, float startTime ) {
        int nextTrack = 0;
        for ( int i = 0; i < musicPlaylist.Length; i++ ) {
            if ( musicPlaylist[i] == trackID ) {
                // Debug.Log( "TrackID: " + i.ToString() );
                nextTrack = i;
                break;
            }
        }
        return _PlayMusicTrackWithID( nextTrack, 0, true, startTime );
    }

    private AudioObject _PlayMusicTrackWithID( int nextTrack, float delay, bool addToPlayedList, float startTime = 0 )
    {
        if ( nextTrack < 0 )
        {
            return null;
        ...
        AudioObject audioObj = _PlayMusic( musicPlaylist[ nextTrack ], 1, delay, startTime );
